            ...
            <div class="col-lg-8 col-md-10 col-12">
                <div class="card shadow-sm" style="border-radius: 15px;">
                    <div class="card-header bg-primary text-white text-center py-3">
                        <h5 class="mb-0 text-white">Tambah Data Sengketa Persil</h5>
                    </div>
                    <div class="card-body p-4">
                        ...
                        {{-- ACTION diubah ke sengketa_persil.store --}}
                        <form action="{{ route('sengketa_persil.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label class="form-label fw-bold">Pihak 1 (Penggugat) <span class="text-danger">*</span></label>
                                    <input type="text" name="pihak_1" class="form-control @error('pihak_1') is-invalid @enderror"
                                        value="{{ old('pihak_1') }}" placeholder="Nama Pihak 1" required>
                                    @error('pihak_1')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6">
                                    <label class="form-label fw-bold">Pihak 2 (Tergugat)</label>
                                    <input type="text" name="pihak_2" class="form-control @error('pihak_2') is-invalid @enderror"
                                        value="{{ old('pihak_2') }}" placeholder="Nama Pihak 2 (Opsional)">
                                    @error('pihak_2')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            {{-- KRONOLOGI --}}
                            <div class="mb-3">
                                <label class="form-label fw-bold">Kronologi Sengketa <span class="text-danger">*</span></label>
                                <textarea name="kronologi" class="form-control @error('kronologi') is-invalid @enderror" rows="3"
                                    placeholder="Jelaskan kronologi sengketa secara detail" required>{{ old('kronologi') }}</textarea>
                                @error('kronologi')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-8">
                                    <label class="form-label fw-bold">Penyelesaian (Opsional)</label>
                                    <input type="text" name="penyelesaian" class="form-control @error('penyelesaian') is-invalid @enderror"
                                        value="{{ old('penyelesaian') }}" placeholder="Hasil penyelesaian (diisi jika selesai)">
                                    @error('penyelesaian')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="mb-4">
                                <label class="form-label fw-bold">Upload Bukti / Lampiran</label>
                            ...
                            <div class="d-flex justify-content-between mt-4">
                                ...
                                <button type="submit" class="btn btn-primary px-5">Laporkan Sengketa</button>
                            </div>
                        </form>
                    </div>
                </div>
...
{{-- JAVASCRIPT PREVIEW (Diambil dari template Dokumen Persil, path disesuaikan jika perlu, tapi logikanya sama) --}}
<script>
